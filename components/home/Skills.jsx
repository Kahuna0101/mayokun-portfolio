...
const Skills = async () => {
  ...
  return (
    <section id="skills" className="flex-app">
      <h2 className="head-text"><span className="text-secondary-color">Skills &</span> Experience</h2>
        ...
        <motion.div className="flex flex-1 flex-wrap justify-start items-start mr-20 max-[900px]:mr-0 max-[900px]:justify-center max-[900px]:items-center">
          {skills.map((skill) => (
            <motion.div
              ...
              key={skill.name}
            >
              <div className={`flex flex-col justify-center items-center w-28 h-28 rounded-[50%] bg-white hover:shadow-lg ${skill.bgColor}`}>
                <Image 
                  src={skill.image}
                  alt={skill.name}
                  width={50}
        ...
        {experiences?.map((experience) => (
              ...
              >
                <div className="mr-12 max-[450px]:mr-4">
                  <p className="bold-text !text-secondary-color !font-extrabold">{experience.year}</p>
                </div>
                <motion.div className="flex-1">
                  {experience.works.map((work) => (
                    ...
                    <motion.div
                      ...
                      key={work.company}
                    >
                      <h4 className="bold-text !font-medium">{work.position}</h4>
                      <p className="p-text bold-text !text-secondary-color !text-xl">{work.company}</p>
